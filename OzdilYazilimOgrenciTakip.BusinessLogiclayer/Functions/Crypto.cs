...
namespace OzdilYazilimOgrenciTakip.BusinessLogiclayer.Functions
{
    public static class Crypto
    {
        ...
        public static string Decrypt(this string sifresiCozulecekVeri, string anahtar)
        {
            ...
            var sifresiCozelecekVeriToByte = System.Convert.FromBase64String(sifresiCozulecekVeri);
            var anahtarToByte = Encoding.UTF8.GetBytes(anahtar);
            ...
            using (var ms=new MemoryStream())
            {
                using (var aes=new RijndaelManaged())
                {
                    ...
                    {
                        cs.Write(sifresiCozelecekVeriToByte, 0, sifresiCozelecekVeriToByte.Length);
                        cs.Close();
                    }
